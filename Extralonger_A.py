...
                local_attn_score = attn_score.masked_fill(~adj_mask, -torch.inf)  
...
        x_s = self.s_input(x_s.transpose(1, 2))  
        features = [x_s]
        if self.spatial_embedding_dim > 0:s 
            spatial_emb = self.node_emb.expand(
                batch_size, *self.node_emb.shape
            )
            features.append(spatial_emb)   
        x_s = torch.cat(features, dim=-1)  

        for attn in self.attn_layers_s:
            x_s = attn(x_s, adj_mask=self.adj_mask)
        x_s = self.s_output(x_s).transpose(1, 2)  
...
        self.dow_embedding_dim = dow_embedding_dim
        self.spatial_embedding_dim = spatial_embedding_dim
S       self.noise_embedding_dim = input_embedding_dim
...
        
        self.t_input  = nn.Sequential(
s           nn.Linear(self.num_nodes, self.input_embedding_dim//2), 
        )
        self.tt_input  = nn.Sequential(
            nn.Linear(self.num_nodes, self.input_embedding_dim//2), 
        )
        
        if in_steps != out_steps:
            self.t_output = nn.Sequential(
...
f       self.s_input  = nn.Linear(self.in_steps, self.model_dim - self.spatial_embedding_dim)
        self.s_output = nn.Linear(self.model_dim, self.out_steps)
...
            ]
        )
        mask_path=r"F:\AI_Bulid\Traffic\OtherPro\Extralonger-main\model\mask_2_20.npy"
s       self.AE=AEmbedFromMask(mask_path)
...
        x_t = self.t_input(x_t) 
        x_tt=self.tt_input(x_t1) 

        x_t=torch.cat([x_t,x_tt],dim=2)

        features = [x_t]
         
        if self.tod_embedding_dim > 0:
s           tod_emb = self.tod_embedding(
                (tod * self.steps_per_day).long() 
            )  
            features.append(tod_emb) 
        if self.dow_embedding_dim > 0:
...
s
        for attn in self.attn_layers_s:
    t       x_s = attn(x_s, adj_mask=self.adj_mask)
        x_s = self.s_output(x_s).transpose(1, 2)  
